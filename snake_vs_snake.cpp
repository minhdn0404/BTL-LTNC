...
struct Dot
{
    ...
    void move()  // chỉ với body[0]
    {
        x+=directionX;
        y+=directionY;
        if (x == SCREEN_WIDTH-20) x = 20;
        if (x == 20-size) x = SCREEN_WIDTH-20-size;
        if (y == SCREEN_HEIGHT-20) y = 40;
        if (y == 40 - size) y = SCREEN_HEIGHT-20-size;
    }
    void  turn_right ()
    {
        directionX = step;
...
struct Snake
{
    ...
    bool eat_other (Snake &other_snake)
    {
